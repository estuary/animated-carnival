...
create table directives (
  ...
  constraint "spec must have a string `type` property" check (
...
create index idx_directives_catalog_prefix on directives
  (lower(catalog_prefix) text_pattern_ops);
...
create policy "Users may select directives which they've applied"
...
User-supplied claims which parameterize the directve''s evaluation.

User claims are initially null and must be supplied by the user
for evaluation of the directive to begin.
';
...
begin
  ...
  select * into directive_row from directives d where d.token = bearer_token;
...
If the supplied token is valid, a new row is created in `applied_directives`
which the must then go on to supply with claims.
